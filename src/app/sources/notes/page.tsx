...
import { AppleNotesIcon } from '@/components/ConnectorIcons';
...
export default function AppleNotesPage() {
  const [phase, setPhase] = useState<'idle' | 'checking' | 'no-access' | 'ready' | 'syncing' | 'done'>('idle');
  const [noteCount, setNoteCount] = useState(0);
  const [syncing, setSyncing] = useState(false);
  const [syncResult, setSyncResult] = useState<{ entriesCreated: number } | null>(null);
  ...
  useEffect(() => {
    ...
    ipc.getArticlesBySource('apple-notes').then((data: any) => setArticles(data || { articlesCreated: [], articlesUpdated: [] })).catch(() => {});
  }, []);

  useEffect(() => {
    if (!ipc) return;
    if (ipc.getPlatform) ipc.getPlatform().then((p: string) => setPlatform(p)).catch(() => setPlatform('unknown'));
  }, []);

  const isMac = platform === 'darwin';

  useEffect(() => {
    if (!ipc || !ipc.checkAppleNotesAccess) return;
    setPhase('checking');
    ipc.checkAppleNotesAccess().then((res: any) => {
      if (res.hasAccess) { setNoteCount(res.noteCount || 0); setPhase('ready'); }
      else { setPhase('no-access'); if (res.error) setError(res.error); }
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      const result = await ipc.listEntries({ sourceType: 'apple-notes' });
  ...
  const handleSync = useCallback(async () => {
    if (!ipc || !ipc.syncAppleNotes) return;
    setSyncing(true);
    setError(null);
    setSyncResult(null);
    try {
      const res = await ipc.syncAppleNotes();
      setSyncResult({ entriesCreated: res.entriesCreated });
      setPhase('done');
      loadEntries();
    } catch (e: any) {
  ...
  return (
    <div className="page-transition" style={{ height: '100%', overflowY: 'auto', fontFamily: 'Inter, sans-serif' }}>
      <header style={{ height: 48, display: 'flex', alignItems: 'center', paddingLeft: 32, paddingRight: 32, gap: 16 }}>
        ...
        <h2 style={{ fontSize: 13, fontWeight: 600, color: '#1b1c1b', fontFamily: 'Newsreader, serif' }}>Apple Notes</h2>
      </header>
      ...
      <div style={{ padding: '0 32px 32px 32px', maxWidth: 960 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {isMac ? (
              <button
                ...
                } : canSync ? handleSync : undefined}
                disabled={syncing || phase === 'idle' || phase === 'checking'}
                style={{
                  padding: '8px 20px',
                  background: syncing || phase === 'idle' || phase === 'checking' ? '#e4e2e0' : '#d97757',
                  color: syncing || phase === 'idle' || phase === 'checking' ? '#88726c' : '#ffffff',
                  borderRadius: 6, fontSize: 13, fontWeight: 600, border: 'none',
                  cursor: syncing || phase === 'idle' || phase === 'checking' ? 'default' : 'pointer',
                }}
              >
                {syncing ? 'Syncing...' : phase === 'no-access' ? 'Check Again' : phase === 'done' ? 'Re-sync' : 'Sync'}
              </button>
            ) : (
              <button
                style={{ padding: '8px 20px', background: '#e4e2e0', color: '#55433d', borderRadius: 6, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}
                onClick={() => {
            ...
            <CompileButton sourceType="apple-notes" />
          </div>
          <span style={{ fontSize: 13, color: '#55433d' }}>
            {entries.length > 0 ? `${entries.length} entries synced` : phase === 'ready' ? `${noteCount} notes available` : phase === 'no-access' ? 'No access' : 'Checking...'}
          </span>
        </div>
          ...
          {showHelp && (
            <div style={{ marginTop: 12, padding: 16, background: '#f5f3f1', borderRadius: 10, fontSize: 13, color: '#55433d', lineHeight: 1.8, fontFamily: 'Inter, sans-serif' }}>
              <ol style={{ paddingLeft: 20, margin: 0 }}>
                <li>Cortex reads your notes directly from the macOS Notes app via AppleScript</li>
                <li>You may see a macOS permission prompt asking to allow Cortex to control Notes</li>
                <li>Click &quot;Allow&quot; when prompted</li>
                <li>All your notes will be synced as source entries</li>
        ...
        {syncResult && (
          <div style={{ background: '#ffffff', borderRadius: 12, padding: 20, marginBottom: 24, border: '1px solid rgba(153,70,42,0.1)', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ fontSize: 14, color: '#55433d' }}>{syncResult.entriesCreated} entries created</span>
          </div>
        )}
        ...
        {entriesLoaded && entries.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 48, paddingBottom: 48, textAlign: 'center' }}>
            <div style={{ marginBottom: 16 }}><AppleNotesIcon size={48} /></div>
            <p style={{ fontSize: 14, color: '#55433d' }}>No Apple Notes entries yet. Sync to get started.</p>
          </div>
        )}
          ...
          <div style={{ background: '#f5f3f1', borderRadius: 12, padding: 20, marginTop: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11, color: '#88726c', marginLeft: 4 }}>
                {articles.articlesCreated.length + articles.articlesUpdated.length} articles created from Apple Notes
              </span>
            </div>
