...
import { ObsidianIcon } from '@/components/ConnectorIcons';
...
export default function ObsidianPage() {
  ...
  const [importResult, setImportResult] = useState<{ entriesCreated: number; filesFound: number } | null>(null);
  ...
  useEffect(() => {
    ...
    ipc.getArticlesBySource('obsidian').then((data: any) => setArticles(data || { articlesCreated: [], articlesUpdated: [] })).catch(() => {});
  }, []);
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      const result = await ipc.listEntries({ sourceType: 'obsidian' });
  ...
  const handleImport = useCallback(async () => {
    if (!ipc || !ipc.importObsidianVault) return;
    setImporting(true);
    setError(null);
    setImportResult(null);
    try {
      const res = await ipc.importObsidianVault();
      if (res.cancelled) { setImporting(false); return; }
      setImportResult({ entriesCreated: res.entriesCreated, filesFound: res.filesFound });
      loadEntries();
    } catch (e: any) {
  ...
  return (
    <div className="page-transition" style={{ height: '100%', overflowY: 'auto', fontFamily: 'Inter, sans-serif' }}>
      <header style={{ height: 48, display: 'flex', alignItems: 'center', paddingLeft: 32, paddingRight: 32, gap: 16 }}>
        ...
        <h2 style={{ fontSize: 13, fontWeight: 600, color: '#1b1c1b', fontFamily: 'Newsreader, serif' }}>Obsidian</h2>
      </header>
      ...
      <div style={{ padding: '0 32px 32px 32px', maxWidth: 960 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <CompileButton sourceType="obsidian" />
          </div>
          ...
          {showHelp && (
            <div style={{ marginTop: 12, padding: 16, background: '#f5f3f1', borderRadius: 10, fontSize: 13, color: '#55433d', lineHeight: 1.8, fontFamily: 'Inter, sans-serif' }}>
              <ol style={{ paddingLeft: 20, margin: 0 }}>
                <li>Click &quot;Import Vault&quot; above</li>
                <li>Select your Obsidian vault folder (the folder containing your .md files)</li>
                <li>Cortex imports all markdown files as source entries</li>
                <li>Wikilinks are preserved in the raw entries</li>
                <li>You can re-import anytime to pick up new notes</li>
              </ol>
            </div>
          )}
        </div>
        ...
        {importResult && (
          <div style={{ background: '#ffffff', borderRadius: 12, padding: 20, marginBottom: 24, border: '1px solid rgba(153,70,42,0.1)', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ fontSize: 14, color: '#55433d' }}>{importResult.filesFound} files imported, {importResult.entriesCreated} entries created</span>
          </div>
        )}
        ...
        {entriesLoaded && entries.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 48, paddingBottom: 48, textAlign: 'center' }}>
            <div style={{ marginBottom: 16 }}><ObsidianIcon size={48} /></div>
            <p style={{ fontSize: 14, color: '#55433d' }}>No Obsidian entries yet. Import a vault to get started.</p>
          </div>
        )}
          ...
          <div style={{ background: '#f5f3f1', borderRadius: 12, padding: 20, marginTop: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11, color: '#88726c', marginLeft: 4 }}>
                {articles.articlesCreated.length + articles.articlesUpdated.length} articles created from Obsidian
              </span>
            </div>
